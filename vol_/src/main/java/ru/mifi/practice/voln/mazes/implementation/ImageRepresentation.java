...
public record ImageRepresentation(int width,
    ...
    private BufferedImage createImage(Maze.Grid maze, Maze.Point[] path) {
        ...
        if (path != null) {
            drawPath(g, path, pathColor, width, pathThickness);
        }
        return result;
    }
...
//        g.setComposite(AlphaComposite.SrcOver);
        ...
        g.drawString(String.format("%04d", index), thickness(), thickness() + g.getFontMetrics().getHeight());
        return result;
    }
    ...
    public void snapshot(int index, Maze.Grid grid, Maze.Point[] points, Color color) {
        BufferedImage image = createSnapshot(index, grid, points, color);
