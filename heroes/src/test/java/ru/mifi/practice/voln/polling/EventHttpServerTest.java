...
class EventHttpServerTest {

    private static final ObjectMapper MAPPER = new ObjectMapper().registerModule(new JavaTimeModule());
    private EventHttpServer server;
    ...
    void setUp() throws Exception {
        ...
        server = new EventHttpServer(0, eventService);
        server.start();
        port = server.getPort();
    ...
    void testPostAndGetEvents() throws Exception {
        ...
        HttpRequest getRequest = HttpRequest.newBuilder()
            .uri(URI.create("http://localhost:" + port + "/events?last-offset=0"))
            .GET()
            .build();
        ...
        List<Event> events = MAPPER.readValue(getResponse.body(), new TypeReference<>() {
        });
    ...
    void testLongPolling() throws Exception {
        CompletableFuture<HttpResponse<String>> futureResponse = client.sendAsync(
            HttpRequest.newBuilder()
                .uri(URI.create("http://localhost:" + port + "/events?last-offset=0&timeout=2"))
                .GET()
                .build(),
    ...
    void testTimeout() throws Exception {
        HttpRequest getRequest = HttpRequest.newBuilder()
            .uri(URI.create("http://localhost:" + port + "/events?last-offset=0&timeout=1"))
            .GET()
            .build();
    ...
    void testSse() throws Exception {
        EventService sseEventService = new EventService.Default();
        EventHttpServer sseServer = new EventHttpServer(0, sseEventService, EventHttpServer.TransportMode.SSE);
        sseServer.start();
